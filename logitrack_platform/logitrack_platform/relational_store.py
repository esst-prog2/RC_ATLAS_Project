...
def ensure_schema(conn: sqlite3.Connection) -> None:
    ...
    for column_name in ("password_salt", "password_hash", "api_token_hash"):
    ...
    try:
        ...
        for user in snapshot.get("users", []) or []:
            ...
            conn.execute(
                """
                INSERT INTO users
                (user_id, username, full_name, email, role, password_salt, password_hash, api_token_hash, is_active, created_at, updated_at, last_login_at, synced_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    str(user.get("id", "")),
